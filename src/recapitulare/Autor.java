...
public class Autor {
    ...
    // constructor un sg parametru: nume
    public Autor(String nume) {
        this.nume = nume;
    }

    // constructor toti parametri - si ia de la primu numele
    ...
    public Pagina[] findPaginiByAutor() {
        ...
        for (int i = 0; i < carti.length; i++) {
            Carte carteCurenta = carti[i]; //i = nr de cartea
            Pagina[] paginiCarteCurent = carteCurenta.getPagini();
            for (int j = 0; j < paginiCarteCurent.length; j++) { // j = indexul de pagina
                Pagina paginaCurenta = paginiCarteCurent[j];
                rezultat[k] = paginaCurenta; //k= stocheaza rezultatul (nr de pagini - pt fiecare carte in parte)
                k++; // k = calculeaza nr de pagini
            }
        }
        return rezultat;
    }
